	...
	while (true)
	{
		for (auto & kvp : MonstersInfo.monsters)
		{
			...
			for (auto & player : CharactersInfo.players)
			{
				...
				if (monster.IsPlayerInHitRange(player.second) && !monster.IsAttacking())
				{
...
void MainIocp::HitMonster(stringstream & RecvStream, stSOCKETINFO * pSocket)
{
	// 몬스터 피격 처리
// 	Monster monster;
// 	monster.Read(RecvStream);
// 	Monsters[monster.Id].Damaged(0.1f);
// 
// 	// 다른 플레이어에게 브로드캐스트
// 	stringstream SendStream;
// 	SendStream << EPacketType::HIT_MONSTER << endl;
// 	Monsters[monster.Id].Write(SendStream);
